...
class DefaultInterfaceTests(unittest.TestCase):
    def test_desktop_default_is_udp_lan(self):
        ...
        with patch("chatx5.utils.platform.is_android", return_value=False):
...
class HubDiscoveryScopeTests(unittest.TestCase):
    def test_pinned_lan_scope_applies_while_hub_client(self):
        ...
        with patch.object(server, "load_settings", return_value=settings):
            with patch("chatx5.web.server.lan_discovery_configured", return_value=True):
...
class HubP2pLinkTests(unittest.TestCase):
    def test_link_needs_failover_keeps_udp_for_p2p_peer(self):
        ...
        backend = MessagingBackend(identity=ident, config_dir="/tmp/chatx5-p2p-test")
...
class HubFailoverTests(unittest.TestCase):
    def test_failover_uses_tcp_for_hub_server_peer(self):
        ...
        ident = MagicMock()
        ident.hash = bytes.fromhex("a" * 32)
        backend = MessagingBackend(identity=ident, config_dir="/tmp/chatx5-hub-test")
        backend.running = True
...
class HubPeerTests(unittest.TestCase):
    ...
    def test_discovery_scope_ip_auto_uses_primary_lan(self):
        ...
        server = ChatWebServer.__new__(ChatWebServer)
        server.config_dir = "/tmp/chatx5-test"
        from unittest.mock import patch
        with patch.object(
            ...
            "chatx5.web.server.detect_lan_ip", return_value="10.10.100.12",
        ):
