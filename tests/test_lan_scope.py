...
class LanScopeTests(unittest.TestCase):
    ...
    def test_beacon_rejects_cross_subnet_peer_ip(self):
        ...
        with patch("chatx5.utils.platform.discovery_scope_ip", return_value="10.10.100.4"):
    ...
    def test_store_peer_rejects_cross_subnet_when_scoped(self):
        ...
        with patch("chatx5.core.discovery.serial_discovery_active", return_value=False):
            with patch("chatx5.core.discovery.PeerDiscovery._scope_ip", return_value="10.10.10.37"):
                disc._store_peer({
                    "hash": "f" * 32,
                    "ip": "10.0.5.10",
                    "name": "UBUNTU",
    ...
    def test_store_peer_rejects_cross_subnet_lan_when_usb_up(self):
        ...
        with patch("chatx5.core.discovery.serial_discovery_active", return_value=True):
            with patch("chatx5.core.discovery.PeerDiscovery._scope_ip", return_value="10.0.30.112"):
                ok = disc._store_peer({
                    "hash": peer_hash,
                    "ip": "10.10.10.2",
                    "name": "13600k",
    ...
    def test_store_peer_drops_existing_peer_on_subnet_move(self):
        ...
        with patch("chatx5.core.discovery.PeerDiscovery._scope_ip", return_value="10.0.5.10"):
            disc._store_peer({
                "hash": peer_hash,
                "ip": "10.10.10.37",
                "name": "ARCH",
    ...
    def test_ipless_rns_peer_rejected_when_serial_active(self):
        ...
        disc = PeerDiscovery()
        with patch("chatx5.core.discovery.serial_discovery_active", return_value=True):
            with patch("chatx5.core.discovery.PeerDiscovery._scope_ip", return_value="10.0.5.37"):
                ok = disc._store_peer({
                    "hash": "s" * 32,
                    "name": "BRIDGEDPEER",
                    "via": "rns",
    ...
    def test_serial_via_peer_bypasses_scope(self):
        ...
        disc = PeerDiscovery()
        with patch("chatx5.core.discovery.serial_discovery_active", return_value=True):
            with patch("chatx5.core.discovery.PeerDiscovery._scope_ip", return_value="10.0.30.2"):
                disc._store_peer({
                    "hash": "t" * 32,
                    "name": "REMOTE",
                    "via": "serial",
    ...
    def test_lan_broadcast_uses_pinned_ip(self):
        ...
        with patch("chatx5.utils.platform.list_network_interfaces", return_value=[]):
            with patch("chatx5.utils.platform.lan_ip", return_value=None):
