...
from chatx5.core.transport_isolation import (
    _filter_interfaces,
...
class TransportIsolationTests(unittest.TestCase):
    ...
    def test_dual_transport_requires_serial_and_lan(self):
        serial = MagicMock()
        with patch("chatx5.core.transport_isolation.serial_interface_online", return_value=serial):
            with patch("chatx5.core.transport_isolation.online_interfaces", return_value=[]):
                self.assertFalse(dual_transport_isolation_enabled())
            with patch("chatx5.core.transport_isolation.online_interfaces", return_value=[MagicMock()]):
    ...
    def test_filter_interfaces_blocks_cross_zone_forwarding(self):
        ...
        with patch(
            ...
            with patch(
                "chatx5.core.transport_isolation.interface_family",
