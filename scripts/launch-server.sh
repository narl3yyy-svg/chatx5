#!/usr/bin/env bash
# Launch chatx5 web server with serial port groups (dialout/uucp) when needed.
set -euo pipefail
...
launch_with_group() {
    local grp="$1"
    shift
    local cmd="cd $(printf '%q' "$DIR") && PYTHONPATH=$(printf '%q' "$DIR") $(printf '%q' "$PYTHON") -m chatx5.web.server"
    local arg
    for arg in "$@"; do
...
main() {
    ...
    STOP_SCRIPT="$DIR/scripts/stop-chatx5.sh"
    ...
    exec env PYTHONPATH="$DIR" "$PYTHON" -m chatx5.web.server "$@"
}
