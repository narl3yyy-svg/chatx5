...
UDEV_RULE="$(cd "$(dirname "$0")" && pwd)/udev/99-chatx5-serial.rules"
if [ -f "$UDEV_RULE" ] && [ ! -f /etc/udev/rules.d/99-chatx5-serial.rules ] && [ -t 0 ]; then
    echo ""
    read -r -p "Install udev rule for USB serial permissions? [Y/n]: " udev_opt
    if [[ ! "$udev_opt" =~ ^[Nn]$ ]]; then
        sudo cp "$UDEV_RULE" /etc/udev/rules.d/99-chatx5-serial.rules
        sudo udevadm control --reload-rules
        sudo udevadm trigger
        echo "Installed /etc/udev/rules.d/99-chatx5-serial.rules"
    fi
fi
