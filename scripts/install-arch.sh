...
echo "chatx5 - Arch Linux Installer"
echo "=============================="
...
install_via_venv() {
    local VENV_DIR="$HOME/.local/share/chatx5/venv"
    mkdir -p "$(dirname "$VENV_DIR")"
    $PYTHON -m venv "$VENV_DIR"
    "$VENV_DIR/bin/pip" install .
    echo
    echo "chatx5 installed in virtualenv at $VENV_DIR"
...
if [ "$PEP668" = true ]; then
    ...
    echo "  2) venv  (virtual environment in ~/.local/share/chatx5/)"
...
echo ""
echo "chatx5 installed!"
echo "Run 'chatx5 --help' to get started."
