...
public class MainActivity extends AppCompatActivity {
    ...
    public void showMessageNotification(String title, String body, String peerHash) {
        Chatx5NotificationHelper.show(title, body, peerHash);
    }
    ...
    private void captureNotificationPeer(Intent intent) {
        ...
        String peer = intent.getStringExtra(Chatx5NotificationHelper.EXTRA_OPEN_PEER);
        if (peer != null && !peer.isEmpty()) {
            pendingOpenPeer = peer.replace(":", "");
            intent.removeExtra(Chatx5NotificationHelper.EXTRA_OPEN_PEER);
        }
    }
    ...
    private void deliverPendingOpenPeer() {
        ...
        String js = "window.onChatx5OpenPeer && window.onChatx5OpenPeer("
    ...
    private void setupWebView() {
        ...
        webView.addJavascriptInterface(new Chatx5Bridge(this), "chatx5Android");
    ...
    public void notifyAudioPermissionGranted() {
        webView.post(() -> webView.evaluateJavascript(
                "window.onChatx5AudioPermissionGranted && window.onChatx5AudioPermissionGranted();",
                null));
    }
    ...
    public void requestAudioPermission() {
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.RECORD_AUDIO)) {
            new AlertDialog.Builder(this)
                    .setTitle("Microphone")
                    .setMessage("chatx5 needs microphone access to record voice notes.")
    ...
    public void openAppSettings() {
        try {
            ...
            Toast.makeText(this, "Open Settings → Apps → chatx5 → Permissions", Toast.LENGTH_LONG).show();
        }
    }
        ...
        if (requestCode == REQ_AUDIO) {
            ...
            if (granted) {
                ...
                new AlertDialog.Builder(this)
                        ...
                        .setMessage("Enable microphone for chatx5 in Android Settings to record voice notes.")
    ...
    private void startForegroundService() {
        try {
            Intent intent = new Intent(this, Chatx5ForegroundService.class);
            intent.setAction(Chatx5ForegroundService.ACTION_START);
    ...
    private synchronized void startPythonServer(boolean debug) {
        ...
        serverStarted = true;
        runOnUiThread(() -> showLoading(debug ? "Starting chatx5 (debug)..." : "Starting chatx5..."));

        new Thread(() -> {
            try {
                if (!Python.isStarted()) {
                    throw new IllegalStateException("Python was not started in Chatx5Application");
                }
                ...
                if (host != null && !host.equals("None")) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        startForegroundService();
                        Toast.makeText(this, "chatx5 ready", Toast.LENGTH_SHORT).show();
                    });
                } else {
    ...
    private void notifyFolderPickError(String message) {
        String js = "window.onChatx5FolderPickError && window.onChatx5FolderPickError("
            + org.json.JSONObject.quote(message) + ")";
        webView.post(() -> webView.evaluateJavascript(js, null));
    }

    private void notifyFolderPicked(String path) {
        String js = "window.onChatx5FolderPicked && window.onChatx5FolderPicked("
            + org.json.JSONObject.quote(path) + ")";
        webView.post(() -> webView.evaluateJavascript(js, null));
    }

    private void notifyFolderSendError(String message) {
        String js = "window.onChatx5FolderSendError && window.onChatx5FolderSendError("
            + org.json.JSONObject.quote(message) + ")";
        webView.post(() -> webView.evaluateJavascript(js, null));
    }

    private void notifyFolderSendOk(String name, long size) {
        String js = "window.onChatx5FolderSendOk && window.onChatx5FolderSendOk("
