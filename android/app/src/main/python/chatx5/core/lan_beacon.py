...
from chatx5.core.discovery import APP_NAME
...
from chatx5.utils.platform import is_android, lan_ip
...
class LanBeacon:
    ...
    def start(self):
        ...
        self._listen_thread = threading.Thread(target=self._listen, name="chatx5-beacon-rx", daemon=True)
        self._listen_thread.start()
        if self.periodic:
            self._periodic_thread = threading.Thread(target=self._periodic, name="chatx5-beacon-tx", daemon=True)
    ...
    def set_periodic(self, enabled, on_periodic=None):
        ...
        if enabled and self.running:
            ...
            self._periodic_thread = threading.Thread(
                target=self._periodic, name="chatx5-beacon-tx", daemon=True,
            )
            self._periodic_thread.start()
    ...
    def _known_peer_ips(self):
        from chatx5.core.lan_rns import known_udp_peer_ips
    ...
    def _listen(self):
        while self.running and self._rx_sock:
            ...
            if msg_type == "probe_ack":
                from chatx5.core.peer_probe import register_probe_ack
