...
class _TeeStream:
    def __init__(self, original, log_path):
        ...
        self._file.write(
            f"\n--- chatx5 debug log {datetime.now().isoformat()} ---\n"
        )
        self._file.flush()
...
def _dir_writable(path):
    try:
        ...
        probe = os.path.join(path, ".chatx5_write_test")
...
def resolve_android_debug_dir():
    ...
    try:
        from chatx5.utils.platform import android_files_dir
...
def _startup_log_tail(max_bytes=8000):
    try:
        ...
        path = os.path.join(base, "chatx5-startup.log")
...
def start_debug_capture():
    """Mirror stdout/stderr to Downloads/chatx5-debug-*.txt on Android."""
    ...
    path = os.path.join(downloads, f"chatx5-debug-{stamp}.txt")
    latest = os.path.join(downloads, "chatx5-debug-latest.txt")
    ...
    try:
        ...
        if logs_dir and os.path.isdir(logs_dir):
            for name in sorted(os.listdir(logs_dir)):
                if not name.startswith("chatx5-debug") or not name.endswith(".txt"):
                    continue
