"""Android entry point — starts the full chatx5 web server in a background thread."""
...
os.environ.setdefault("CHATX5_ANDROID", "1")
...
def _startup_log_path():
    base = os.environ.get("CHATX5_FILES_DIR") or "."
    return os.path.join(base, "chatx5-startup.log")
...
def set_debug_mode(flag="0"):
    ...
    if str(flag).strip().lower() in ("1", "true", "yes", "on"):
        os.environ["CHATX5_DEBUG"] = "1"
    else:
        os.environ.pop("CHATX5_DEBUG", None)
...
def start_server():
    ...
    try:
        from chatx5.utils.platform import android_files_dir, is_android
        files_dir = android_files_dir()
        if files_dir:
            os.environ["CHATX5_FILES_DIR"] = files_dir
        if not is_android():
            os.environ["CHATX5_ANDROID"] = "1"
    except Exception as e:
        _startup_log(f"platform init failed: {e}")
        return "None", f"Platform init: {type(e).__name__}: {e}"

    debug_mode = os.environ.get("CHATX5_DEBUG") == "1"
    _startup_log(f"debug_mode={debug_mode}")

    if debug_mode:
        try:
            from chatx5.utils.debug_log import start_debug_capture
    ...
    thread = threading.Thread(target=_run, name="chatx5-server", daemon=True)
    thread.start()
