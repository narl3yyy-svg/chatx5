"""Portable desktop launcher — double-click chatx5.exe (Windows) or chatx5.app (macOS)."""
...
def main():
    root = portable_root()
    os.environ.setdefault("CHATX5_PORTABLE", root)
    os.chdir(root)
    ensure_rns_interfaces()
    ...
    threading.Thread(target=_open_browser, name="chatx5-browser", daemon=True).start()

    ip_hint = "ifconfig" if sys.platform == "darwin" else "ipconfig"
    print(f"chatx5 v{app_version} (portable)")
    print(f"Web UI:  http://127.0.0.1:{port}")
    print(f"LAN UI:  http://<your-ip>:{port}  (see {ip_hint})")
    print(f"Data:    {os.path.join(root, 'chatx5-data')}")
