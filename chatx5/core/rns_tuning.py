"""chatx5-wide RNS performance tuning (private chatx5 network only)."""
...
def patch_tcp_server_ifac_netname():
    ...
    try:
        ...
        return
    if getattr(TCPServerInterface, "_chatx5_ifac_patch", False):
        return
    ...
    TCPServerInterface._chatx5_ifac_patch = True


def apply_chatx5_rns_tuning():
    """Raise RNS MTU above the 500B default for much faster LAN file transfers."""
    patch_tcp_server_ifac_netname()
    try:
        from chatx5.core.transport_isolation import apply_transport_isolation
