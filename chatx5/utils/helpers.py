...
from chatx5.utils.platform import is_android, storage_root


def portable_data_root():
    """When set, all config/data lives beside the portable app folder."""
    env = os.environ.get("CHATX5_PORTABLE")
    if env:
        return os.path.join(env, "chatx5-data")
    if getattr(sys, "frozen", False):
        return os.path.join(os.path.dirname(os.path.abspath(sys.executable)), "chatx5-data")
    return None
...
def get_config_dir():
    ...
    if xdg:
        return os.path.join(xdg, "chatx5")
    return os.path.join(os.path.expanduser("~"), ".config", "chatx5")
...
def get_data_dir():
    ...
    if xdg:
        return os.path.join(xdg, "chatx5")
    return os.path.join(os.path.expanduser("~"), ".local", "share", "chatx5")
