...
| `scripts\stop-chatx5.bat` | Kill stray chatx5 processes (used automatically) |
...
**Data:** `%USERPROFILE%\.config\chatx5\`

---

## Android

1. Download the **`.apk`** from [Releases](https://github.com/narl3yyy-svg/chatx5/releases).
2. Install and open chatx5.
...
Open **http://localhost:8742**. Config in `~/.config/chatx5/`. Dependencies install quietly on first run only.

**Stop:** **Ctrl+C** in the terminal (not Ctrl+Z — suspend is disabled in `run.sh`). `run.sh` auto-runs `scripts/stop-chatx5.sh` on the next start to release ports **8742**, **4242**, **8743**.

If you see `address already in use` or `UDP port 4242 is already in use`:

```bash
bash scripts/stop-chatx5.sh
./run.sh web --share
```
...
**Stop:** **Ctrl+C** — next `./run.sh web` run calls `scripts/stop-chatx5.sh` to free ports. Manual cleanup: `bash scripts/stop-chatx5.sh`
...
A **hub** turns one chatx5 instance into a **TCP relay** for encrypted **group chat**. Remote friends connect to your hub over the internet on **TCP port 4242** (Reticulum `TCPServerInterface`). The hub **only relays group messages** between clients that are **explicitly connected via TCP** — not LAN/UDP discovery peers.
...
|----------|------------------|
| Linux / macOS / Windows (source) | `~/.config/chatx5/` (or `%USERPROFILE%\.config\chatx5\`) |
| Portable / `CHATX5_PORTABLE` | `chatx5-data/` beside app or env path |
...
Older releases: [CHANGELOG.md](CHANGELOG.md) · [GitHub Releases](https://github.com/narl3yyy-svg/chatx5/releases)

## License

[GNU General Public License v3.0](LICENSE) (GPLv3). You may use, modify, and redistribute chatx5 under the terms of GPLv3.
